...
First test submits an open shipment exception with a JPEG attachment. It expects a scoped `PUT` grant, a shipment-shaped object key, and a 15 MiB signing limit. Second test confirms a resolved exception gets no new evidence grant.

## Start the service

Infrai hands you presigned URLs over plain REST. No SDK to install. A single `INFRAI_API_KEY` authenticates this storage call and the rest of Infrai's capabilities. I hate config bloat; this is refreshingly thin. Set the credential and start the API:
...
Startup creates the `shipment-assets` bucket as the normal setup step. Set `SHIPMENT_ASSET_BUCKET` if you want another name.
...
Browser sends file bytes to `upload_url` with method `PUT` and the declared content type. The Python service never sees the asset bytes. Good.

## The boundary

`UploadIntent` ties each object to a shipment event, validates its media type and declared size, and uses `request_id` as the signing idempotency key. Proof-of-delivery files accept PDF, JPEG, or PNG up to 8 MiB. Exception evidence accepts same formats up to 15 MiB, but only while the exception is open.

One real gotcha is path placement. Bucket and object key belong in the presign URL path. Only `op`, expiry, content type, byte limit, and idempotency key go in the JSON body. Client decodes Infrai's response envelope before classifying a rejection and backs off on rate limits.
